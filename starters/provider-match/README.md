# provider-match  —  U-CS 40

**AI Service Matching** · Wasaa Lifestyle

For a specific household, rank the providers most likely to finish the job well.
...
**Call it `provider-match` everywhere** — your repo name, your service URL, your route, and the
variable Benjamin sets when integrating you: `PROVIDER_MATCH_SERVICE_URL`

## Who you depend on

U-CS 39 (marketplace-search). Agree the boundary with them on day one.
...
Higher completion rate at position one than "nearest with best rating", on held-out bookings.
...
docker build -t provider-match . && docker run -p 8000:8000 provider-match
```
