# emergency-triage  —  U-CS 45

**Emergency Response** · Wasaa Lifestyle

Decide how urgent it is, send it to the right person, and never lose it quietly.
...
**Call it `emergency-triage` everywhere** — your repo name, your service URL, your route, and the
variable Benjamin sets when integrating you: `EMERGENCY_TRIAGE_SERVICE_URL`

## Who you depend on

Nothing. Clear rules beat a model — there is almost no history.
...
It escalates when nobody answers, falls back when the app path fails, and never drops silently.
...
docker build -t emergency-triage . && docker run -p 8000:8000 emergency-triage
```
