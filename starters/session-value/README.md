# session-value  —  U-CS 5

**Session Effectiveness Predictor** · Career Mentor OS

Before a session happens, predict whether it will help.
...
**Call it `session-value` everywhere** — your repo name, your service URL, your route, and the
variable Benjamin sets when integrating you: `SESSION_VALUE_SERVICE_URL`

## Who you depend on

U-CS 2 (matching) for the pairings.
...
Only features known BEFORE the session — guard this, it is the main way this project fails.
...
docker build -t session-value . && docker run -p 8000:8000 session-value
```
